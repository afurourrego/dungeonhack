...
Dungeon Flip uses **OCT native token** for its reward economy. Players pay a 0.01 OCT entry fee per run which is automatically distributed across three destinations:
...
- Equals 50,000,000 MIST (OCT has 9 decimals)
...
[fee_distributor.move]
       ↓
├─ 0.035 OCT → Rewards Pool (for weekly distribution)
├─ 0.010 OCT → Dev Balance (accumulates)
└─ 0.005 OCT → Marketing Balance (accumulates)
```
...
| Rank | Percentage | Example (10 OCT pool) |
|------|------------|-----------------------|
| 🥇 1st | 30% | 3.0 OCT |
| 🥈 2nd | 20% | 2.0 OCT |
| 🥉 3rd | 15% | 1.5 OCT |
...
- OneChain contracts cannot query external data sources
...
- **Dev Balance**: 20% of each 0.01 OCT fee = 0.01 OCT per entry
- **Marketing Balance**: 10% of each 0.01 OCT fee = 0.005 OCT per entry
...
console.log(`Dev Treasury: ${devBalance / 1_000_000_000} OCT`);
console.log(`Marketing: ${marketingBalance / 1_000_000_000} OCT`);
```
...
sui client call \
  ...
  # 1000000000 = 1 OCT (with 9 decimals)
```
...
Anyone can view withdrawal history on-chain using OneChain Explorer.
...
- 100 players × 0.01 OCT = 5 OCT total

**Automatic Distribution:**
- Rewards Pool: 3.5 OCT (goes to top 10 players after 1 week)
- Dev Treasury: 1 OCT (accumulated, withdrawable)
- Marketing: 0.5 OCT (accumulated, withdrawable)

**Winner Payouts (After 1 Week):**
- 1st place: 1.05 OCT (30% of 3.5 OCT)
- 2nd place: 0.70 OCT
- 3rd place: 0.525 OCT
- 4th-10th: Decreasing amounts
- Total distributed: 3.5 OCT

**Admin Can Withdraw:**
- Dev: 1 OCT anytime
- Marketing: 0.5 OCT anytime
...
**Distribution:**
- Rewards Pool: 35 OCT
  - 1st place wins 10.5 OCT (30%)
  - Top 10 split 35 OCT
- Dev: 10 OCT accumulated
- Marketing: 5 OCT accumulated
...
- ✅ No reentrancy vulnerabilities (OneChain Move protections)
